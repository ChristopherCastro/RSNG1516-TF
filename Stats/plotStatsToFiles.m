...
if exist('runSetsResults','var')
    ...
    %         fprintf('    · Event Generator (Service Time): %d\n', runSetConf.tmService);
            ...
            plot(bottomRight,runSetStats{1}.time, runSetStats{1}.meanRhoResources,...
                runSetStats{2}.time, runSetStats{2}.meanRhoResources,...
                runSetStats{3}.time, runSetStats{3}.meanRhoResources,...
                runSetStats{4}.time, runSetStats{4}.meanRhoResources,...
                runSetStats{5}.time, runSetStats{5}.meanRhoResources,...
                runSetStats{6}.time, runSetStats{6}.meanRhoResources,...
                runSetStats{7}.time, runSetStats{7}.meanRhoResources,...
                runSetStats{8}.time, runSetStats{8}.meanRhoResources,...
                runSetStats{9}.time, runSetStats{9}.meanRhoResources,...
                runSetStats{10}.time, runSetStats{10}.meanRhoResources);
            title(bottomRight,'Rho medio');
            xlabel(bottomRight,'tiempo');
            ylabel(bottomRight,'Rho medio');
            for j=1:10
                if ~isempty(runSetStats{j}.time())
                    text(runSetStats{j}.time(end),runSetStats{j}.meanRhoResources(end),num2str(runSetStats{j}.meanRhoResources(end)),'Parent', bottomRight);
                end
            end
